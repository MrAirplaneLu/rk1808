...
{
    const int MODEL_IN_WIDTH = 100;
    const int MODEL_IN_HEIGHT = 100;
    ...
    for (int i = 0; i < io_num.n_output; i++)
    {
		...
		printf("outputs[i].buf=%.12f\n", &buffer[2]);
		printf("outputs[i].buf=%f\n", *buffer);
		printf("outputs[i].buf=%f\n", fMaxProb[2]);
		
		for(int temp=0; temp<sz; temp++){cout<<"outputs[i].buf="<<*(buffer+temp)<<endl;}
